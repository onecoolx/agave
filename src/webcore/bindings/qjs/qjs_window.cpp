...
ScriptInterpreter* Window::interpreter() const
{
    ...
    return frame->script()->interpreter();
}
...
{
    ASSERT(p);
    if (ScriptController *script = p->script())
        return script->interpreter()->globalObject(); // the Global object is the "window"
  
    return JS_UNDEFINED; // This can happen with JS disabled on the domain of that window
}

JSValue Window::location(JSContext* ctx) const
{
    if (JS_IsNull(d->loc)) {
...
static HashMap<String, String> parseModalDialogFeatures(JSContext* ctx, JSValue featuresArg)
{
    ...
    Vector<String> features = valueToStringWithUndefinedOrNullCheck(ctx, featuresArg).split(';');
...
static void setWindowFeature(const String& keyString, const String& valueString, WindowFeatures& windowFeatures)
{
    ...
    if (keyString == "left" || keyString == "screenx") {
        ...
        windowFeatures.x = (float)value;
    } else if (keyString == "top" || keyString == "screeny") {
        windowFeatures.ySet = true;
        windowFeatures.y = (float)value;
    } else if (keyString == "width" || keyString == "innerwidth") {
        windowFeatures.widthSet = true;
        windowFeatures.width = (float)value;
    } else if (keyString == "height" || keyString == "innerheight") {
        windowFeatures.heightSet = true;
        windowFeatures.height = (float)value;
    } else if (keyString == "menubar")
        windowFeatures.menuBarVisible = value?true:false;
    else if (keyString == "toolbar")
        windowFeatures.toolBarVisible = value?true:false;
    else if (keyString == "location")
        windowFeatures.locationBarVisible = value?true:false;
    else if (keyString == "status")
        windowFeatures.statusBarVisible = value?true:false;
    else if (keyString == "resizable")
        windowFeatures.resizable = value?true:false;
    else if (keyString == "fullscreen")
        windowFeatures.fullscreen = value?true:false;
    else if (keyString == "scrollbars")
        windowFeatures.scrollbarsVisible = value?true:false;
}
...
{
    return (Frame*)JS_GetOpaque(val, Location::js_class_id);
}
...
{
    Frame* frame = (Frame*)JS_GetOpaque2(ctx, this_val, Location::js_class_id);
    if (!frame)
        return JS_ThrowTypeError(ctx, "Type error"); 
