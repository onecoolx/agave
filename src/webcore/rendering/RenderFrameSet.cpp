...
void RenderFrameSet::paint(PaintInfo& paintInfo, int tx, int ty)
{
    ...
    if (view()->frameView()->frame()->page()->isSmallView()) {
        rows *= cols;
        cols = 1;
    }
...
void RenderFrameSet::layout()
{
    ...
    if (!parent()->isFrameSet()) {
        ...
        if (v->frame()->page()->isSmallView()) {
            m_width = (int)((float)v->frame()->page()->smallViewWidth()*style()->effectiveZoom())-4;
            m_height = v->visibleHeight()*frameSet()->totalCols()*frameSet()->totalRows();
            v->setVScrollbarMode(ScrollbarAuto);
        } else {
    ...
    int borderThickness = frameSet()->border();
    if (view()->frameView()->frame()->page()->isSmallView()) {
        int later_rows = frameSet()->totalCols() * frameSet()->totalRows();

        Length later_c(m_width, Fixed);
        Length *later_r = new Length[later_rows];
        for (int i=0; i<later_rows; i++)
            later_r[i].setValue(Relative, 1);

        layOutAxis(m_rows, (Length*)(&later_r), m_height - (rows - 1) * borderThickness);
        layOutAxis(m_cols, (Length*)(&later_c), m_width - (cols - 1) * borderThickness);
		delete later_r;
    } else {
