...
static void cacluateBackgroundSize(const BackgroundLayer* bgLayer, int& scaledWidth, int& scaledHeight, RenderStyle* style)
{
    ...
    if (bgLayer->isBackgroundSizeSet()) {
        ...
        if (bgWidth.isPercent())
            ...
            if (bgHeight.isPercent()) {
                ...
                scaledWidth = bg->imageSize(style->effectiveZoom()).width() * scaledH / bg->imageSize(style->effectiveZoom()).height();
            } else if (bgHeight.isFixed())
                scaledWidth = bg->imageSize(style->effectiveZoom()).width() * bgHeight.value() / bg->imageSize(style->effectiveZoom()).height();
        }
        ...
        if (bgHeight.isPercent())
            ...
            // scale to maintain our aspect ratio.
            if (bgWidth.isPercent())
                scaledHeight = bg->imageSize(style->effectiveZoom()).height() * scaledWidth / bg->imageSize(style->effectiveZoom()).width();
            else if (bgWidth.isFixed())
                scaledHeight = bg->imageSize(style->effectiveZoom()).height() * bgWidth.value() / bg->imageSize(style->effectiveZoom()).width();
            else if (bgWidth.isAuto()) {
                // If both width and height are auto, we just want to use the image's
                // intrinsic size.
                scaledWidth = bg->imageSize(style->effectiveZoom()).width();
                scaledHeight = bg->imageSize(style->effectiveZoom()).height();
            }
        }
    } else {
        scaledWidth = bg->imageSize(style->effectiveZoom()).width();
...
void RenderBox::calculateBackgroundImageGeometry(const BackgroundLayer* bgLayer, int tx, int ty, int w, int h, IntRect& destRect, IntPoint& phase, IntSize& tileSize)
{
    ...
    cacluateBackgroundSize(bgLayer, scaledImageWidth, scaledImageHeight, style());
