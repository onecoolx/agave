...
void RenderImage::layout()
{
	...
	if (m_cachedImage && m_cachedImage->imageSize(style()->effectiveZoom()) != intrinsicSize())
    	imageChanged(m_cachedImage);
...
bool RenderImage::nodeAtPoint(const HitTestRequest& request, HitTestResult& result, int _x, int _y, int _tx, int _ty, HitTestAction hitTestAction)
{
    ...
    if (inside && node()) {
        if (HTMLMapElement* map = imageMap()) {
            ...
            int mapX = roundf((_x - _tx - this->xPos() - contentBox.x()) / zoom);
            int mapY = roundf((_y - _ty - this->yPos() - contentBox.y()) / zoom);
