...
void RenderLayer::resize(const PlatformMouseEvent& evt, const IntSize& oldOffset)
{
    ...
    if (difference.width()) {
        if (element && element->isControl()) {
            ...
            style->setProperty(CSS_PROP_MARGIN_LEFT, String::number(renderer->marginLeft() / zoomFactor) + "px", false, ec);
            style->setProperty(CSS_PROP_MARGIN_RIGHT, String::number(renderer->marginRight() / zoomFactor) + "px", false, ec);
        }
    ...
    if (difference.height()) {
        if (element && element->isControl()) {
            // Make implicit margins from the theme explicit (see <http://bugs.webkit.org/show_bug.cgi?id=9547>).
            style->setProperty(CSS_PROP_MARGIN_TOP, String::number(renderer->marginTop() / zoomFactor) + "px", false, ec);
            style->setProperty(CSS_PROP_MARGIN_BOTTOM, String::number(renderer->marginBottom() / zoomFactor) + "px", false, ec);
        }
