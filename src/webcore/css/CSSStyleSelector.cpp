...
CSSStyleSelector::CSSStyleSelector(Document* doc, const String& userStyleSheet, StyleSheetList *styleSheets, bool _strictParsing)
{
    ...
    m_selectorAttrs = new HashSet<AtomicStringImpl*>;
}
...
#if 1 //MaCross added
    if (m_document->view()->frame()->page()->isSmallView())
        adjustSmallRenderStyle(style, e);
#endif
...
void CSSStyleSelector::adjustSmallRenderStyle(RenderStyle* style, Element *e)
{
    // These rules are refer from minimo implements.
    if (style->display() != NONE && e) {
        int max_width = (int)((float)m_document->view()->frame()->page()->smallViewWidth()*style->effectiveZoom())-4;

		Length minW = style->minWidth();
		if (minW.isFixed() && minW.value()>max_width)
			style->setMinWidth(Length(max_width, Fixed));

        style->setMaxWidth(Length(max_width, Fixed));
        ...
        style->setPaddingLeft(Length(0, Fixed));
        style->setPaddingRight(Length(0, Fixed));
        style->setMarginTop(Length(0, Fixed));
        style->setMarginBottom(Length(0, Fixed));
        style->setMarginLeft(Length(0, Fixed));
        style->setMarginRight(Length(0, Fixed));
        style->setWhiteSpace(NORMAL);
		style->clearBackgroundLayers();

        if (e->hasTagName(tableTag) || 
            e->hasTagName(tbodyTag) || 
            e->hasTagName(theadTag) ||
            e->hasTagName(tfootTag) ||
            e->hasTagName(trTag)    ||
            e->hasTagName(tdTag)    ||
            e->hasTagName(thTag)    ||
            e->hasTagName(colTag)   ||
            e->hasTagName(colgroupTag))
        {
            style->setDisplay(BLOCK);
        }

        //FIXME: image ?
        if (e->hasTagName(liTag)){
            style->setListStylePosition(INSIDE);
            style->setDisplay(LIST_ITEM);
        }
        if (e->hasTagName(aTag)){
            style->setTextDecoration(UNDERLINE);
            style->accessBackgroundLayers()->clearBackgroundImage();
        }
        if (e->hasTagName(ulTag) || e->hasTagName(olTag)){
            style->setDisplay(BLOCK);
        }
    }
}
...
// length
	...
	case CSS_PROP_ZOOM:
	{
        ...
        // zoom in effect.
        style->setEffectiveZoom(parentStyle ? parentStyle->effectiveZoom() : RenderStyle::initialZoom());
        ...
        if (primitiveValue->getIdent() == CSS_VAL_NORMAL) {
            style->setZoom(RenderStyle::initialZoom());
        } else if (type == CSSPrimitiveValue::CSS_PERCENTAGE) {
            if (primitiveValue->getFloatValue())
                style->setZoom(primitiveValue->getFloatValue() / 100.0f);
        } else if (type == CSSPrimitiveValue::CSS_NUMBER) {
            if (primitiveValue->getFloatValue())
                style->setZoom(primitiveValue->getFloatValue());
        }
...
// length, percent, number
    case CSS_PROP_LINE_HEIGHT:
    {
        ...
        if (primitiveValue->getIdent() == CSS_VAL_NORMAL)
            ...
            double multiplier = style->effectiveZoom();
...
// shorthand properties
    ...
    case CSS_PROP_OUTLINE_OFFSET:
        ...
        style->setOutlineOffset(primitiveValue->computeLengthInt(style, zoomFactor));
        return;
    ...
    case CSS_PROP_ALIGN_SELF:
        ...
        if (!primitiveValue) return;
        switch (primitiveValue->getIdent()) {
            case CSS_VAL_AUTO: style->setAlignSelf(AlignSelfAuto); break;
            case CSS_VAL_FLEX_START: style->setAlignSelf(AlignSelfFlexStart); break;
            case CSS_VAL_FLEX_END: style->setAlignSelf(AlignSelfFlexEnd); break;
    ...
    case CSS_PROP_ORDER:
        ...
        if (!primitiveValue || primitiveValue->primitiveType() != CSSPrimitiveValue::CSS_NUMBER)
            return;
        style->setFlexOrder((int)primitiveValue->getFloatValue());
        return;
    case CSS_PROP_FLEX:
    ...
    case CSS_PROP__WEBKIT_COLUMN_GAP: {
        ...
        style->setColumnGap(primitiveValue->computeLengthFloat(style, zoomFactor));
        return;
    }
    case CSS_PROP__WEBKIT_COLUMN_WIDTH: {
        ...
        style->setColumnWidth(primitiveValue->computeLengthFloat(style, zoomFactor));
        return;
    }
    ...
    case CSS_PROP__WEBKIT_TEXT_STROKE_WIDTH: {
        ...
        switch (primitiveValue->getIdent()) {
            ...
            case CSS_VAL_THICK: {
                ...
                width = val.computeLengthFloat(style, zoomFactor);
                break;
            }
            default:
...
float CSSStyleSelector::getComputedSizeFromSpecifiedSize(bool isAbsoluteSize, float specifiedSize)
{
    ...
    float zoomPercent = style->effectiveZoom();
	if (m_document->frame())
    	zoomPercent *= m_document->frame()->zoomFactor() / 100.0f;

    float zoomedSize = specifiedSize * zoomPercent;

    // Apply the hard minimum first.  We only apply the hard minimum if after zooming we're still too small.
    if (zoomedSize < minSize)
        zoomedSize = (float)minSize;
    ...
    if (zoomedSize < minLogicalSize && (specifiedSize >= minLogicalSize || !isAbsoluteSize))
        zoomedSize = (float)minLogicalSize;
...
float CSSStyleSelector::fontSizeForKeyword(int keyword, bool quirksMode, bool fixed) const
{
    ...
    if (mediumSize >= fontSizeTableMin && mediumSize <= fontSizeTableMax) {
        ...
        return (float)(quirksMode ? quirksFontSizeTable[row][col] : strictFontSizeTable[row][col]);
    }
    
    // Value is outside the range of the table. Apply the scale factor instead.
    float minLogicalSize = (float)max(settings->minimumLogicalFontSize(), 1);
