...
static png_voidp PNGAPI png_malloc(png_structp, png_size_t s) {
	return fastMalloc(s);
}
static void PNGAPI png_free(png_structp, png_voidp p) {
	fastFree(p);
}
...
public:
    ...
    {
        m_png = png_create_read_struct_2(PNG_LIBPNG_VER_STRING, NULL, decodingFailed, decodingWarning , 0, png_malloc, png_free);
...
PixelsBuffer* PNGImageDecoder::frameBufferAtIndex(size_t index)
{
    if (index)
        return 0;
    ...
    PixelsBuffer& frame = m_frameBufferCache[0];
...
void PNGImageDecoder::decode(bool sizeOnly)
{
    ...
    if (m_failed || (!m_frameBufferCache.isEmpty() && m_frameBufferCache[0].status() == PixelsBuffer::FrameComplete)) {
        delete m_reader;
        m_reader = 0;
        m_data.clear();
    }
...
void PNGImageDecoder::rowAvailable(unsigned char* rowBuffer, unsigned rowIndex, int interlacePass)
{
    ...
    */
    ...
    for (unsigned i = 0; i < (unsigned)width; i++) {
        ...
        PixelsBuffer::setRGBA(rgba, red, green, blue, alpha);
        buffer.bytes().setPixel(i + dst, rgba);
