...
void Element::scrollByUnits(int units, ScrollGranularity granularity)
{
    ...
    if (RenderObject *rend = renderer()) {
        if (rend->hasOverflowClip()) {
            ...
            rend->layer()->scroll(direction, granularity, (float)units);
        }
    }
}
...
int Element::clientWidth()
{
    ...
    if ((!inCompatMode && document()->documentElement() == this) ||
        ...
        if (FrameView* view = document()->view()) {
            if (RenderObject* rend = document()->renderer())
                return adjustForAbsoluteZoom(view->visibleWidth(), rend);
		}
    }
...
int Element::clientHeight()
{
    ...
    if ((!inCompatMode && document()->documentElement() == this) ||
        ...
        if (FrameView* view = document()->view()) {
            if (RenderObject* rend = document()->renderer())
                return adjustForAbsoluteZoom(view->visibleHeight(), rend);
		}
    }
...
void Element::recalcStyle(StyleChange change)
{
    ...
    bool hasParentStyle = (parentNode() && parentNode()->renderStyle()) ? true : false;
...
bool Element::hasAttribute(const String& name) const
{
    ...
    if (!attrs)
        return false;
    String localName = shouldIgnoreAttributeCase(this) ? name.lower() : name;
    return attrs->getAttributeItem(localName) ? true : false;
}
...
bool Element::hasAttributeNS(const String& namespaceURI, const String& localName) const
{
    ...
    if (!attrs)
        return false;
    return attrs->getAttributeItem(QualifiedName(nullAtom, localName, namespaceURI)) ? true : false;
}
