...
public:
    ...
    void appendErrorCallback(XMLTokenizer::ErrorType type, const char* message, int lineNumber, int columnNumber)
    {
        ...
        callback->message = fastStrdup(message);
        callback->type = type;
...
XMLTokenizer::XMLTokenizer(DocumentFragment* fragment, Element* parentElement)
    ...
    , m_currentNodeIsReferenced(fragment?true:false)
    , m_sawError(false)
    , m_sawXSLTransform(false)
    , m_sawFirstElement(false)
    , m_isXHTMLDocument(false)
...
void XMLTokenizer::end()
{
    ...
    if (m_sawError)
        ;//insertErrorMessageBlock();//macross change ,don't insert this block.
    else {
        exitText();
        m_doc->updateStyleSelector();
    }
