...
Element* Document::elementFromPoint(int x, int y) const
{
    ...
    float zoomFactor = m_frame->pageZoomFactor();
    IntPoint point = IntPoint(roundf((float)x * zoomFactor  + view()->contentsX()), roundf((float)y * zoomFactor + view()->contentsY()));
...
bool Document::inDesignMode() const
{
    for (const Document* d = this; d; d = d->parentDocument()) {
        if (d->m_designMode != inherit)
			return d->m_designMode ? true : false;
    }
    return false;
}
...
void Document::setAsync(bool async, ExceptionCode& ec)
{
    if (!implementation()->hasFeature("XML", String())) {
        ec = NOT_SUPPORTED_ERR;
        return;
