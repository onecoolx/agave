...
void Chrome::setWindowFeature(const WindowFeatures& features)const
{
    m_client->setWindowFeature(features);
}
...
bool Chrome::toolbarVisible() const
{
...
bool Chrome::locationbarVisible() const
{
    return m_client->locationbarVisible();
}

bool Chrome::personalbarVisible() const
{
...
#if 1
void Chrome::focusChangedRect(const IntRect& r)
{
	m_client->focusChangedRect(r);
}
#endif

void Chrome::setToolTip(const HitTestResult& result, const IntPoint& origin_point)
{
    ...
    m_client->setToolTip(toolTip, origin_point);
}
