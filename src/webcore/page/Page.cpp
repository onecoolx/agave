...
#endif
#if ENABLE(FTPDIR)    
    m_settings->setFTPDirectoryTemplatePath("/var/tmp/ftplist");
#endif
}
...
void Page::setSmallView(bool smallView)
{
    m_inSmallView = smallView;
}

bool Page::isSmallView (void)
{
    return m_inSmallView;
}

void Page::setSmallViewWidth(unsigned int width)
{
    m_smallViewWidth = width;
}

unsigned Page::smallViewWidth(void)
{
    return m_smallViewWidth;
}
