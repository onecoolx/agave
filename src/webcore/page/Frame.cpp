...
float Frame::pageZoomFactor() const
{
  return d->m_pageFactor;
}

bool Frame::setPageZoomFactor(float scale)
{
	if (d->m_pageFactor == scale)
      return false;
	...
	for (Frame* child = tree()->firstChild(); child; child = child->tree()->nextSibling())
		child->setPageZoomFactor(d->m_pageFactor);
...
bool Frame::setZoomFactor(int percent)
{  
...
#if 1
void Frame::notifyFocusChanged(const Element* node)
{
	Element* n = const_cast<Element*>(node);
	d->m_page->chrome()->focusChangedRect(IntRect(n->offsetLeft(), 
				n->offsetTop(), n->offsetWidth(), n->offsetHeight()));
}
#endif
...
FramePrivate::FramePrivate(Page* page, Frame* parent, Frame* thisFrame, HTMLFrameOwnerElement* ownerElement,
	...
	, m_pageFactor(parent ? parent->d->m_pageFactor : 1.0f)
