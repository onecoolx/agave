...
void PageManager::draw_pageBtns(ps_context* gc, const Rect& rc)
{
    ...
    if (rt.intersect(rc)) {
        ...
        p[0].x = ur.x + ur.w / 2;
        p[0].y = ur.y + ur.h / 10 * 3;
        p[1].x = ur.x + ur.w / 10 * 3;
        p[1].y = ur.y + ur.h / 10 * 7;
        p[2].x = ur.x + ur.w / 2;
        p[2].y = ur.y + ur.h / 5 * 3;
        p[3].x = ur.x + ur.w / 10 * 7;
        p[3].y = ur.y + ur.h / 10 * 7;
        ps_move_to(gc, &p[0]);
        ps_line_to(gc, &p[1]);
        ps_line_to(gc, &p[2]);
        ps_line_to(gc, &p[3]);
    ...
    if (rt.intersect(rc)) {
        ...
        p[0].x = dr.x + dr.w / 2;
        p[0].y = dr.y + dr.h - dr.h / 10 * 3;
        p[1].x = dr.x + dr.w / 10 * 3;
        p[1].y = dr.y + dr.h - dr.h / 10 * 7;
        p[2].x = dr.x + dr.w / 2;
        p[2].y = dr.y + dr.h - dr.h / 5 * 3;
        p[3].x = dr.x + dr.w / 10 * 7;
        p[3].y = dr.y + dr.h - dr.h / 10 * 7;
        ps_move_to(gc, &p[0]);
        ps_line_to(gc, &p[1]);
        ps_line_to(gc, &p[2]);
        ps_line_to(gc, &p[3]);
