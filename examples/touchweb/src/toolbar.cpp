...
static void draw_zoom_btn(ps_context* gc, const ps_rect& rt, MainWindow* main)
{
	...
	p1.x = tr.x + 5*b; p1.y = tr.y + tr.h/2;
	p2.x = tr.x + tr.w - 5*b; p2.y = tr.y + tr.h/2;
	ps_move_to(gc, &p1);
	ps_line_to(gc, &p2);

	WebView * view = main->getTabs()->getActiveView();
	if (view && view->zoomFactor() == 1.0f) {
		p1.x = tr.x + tr.w/2; p1.y = tr.y + 5*b;
		p2.x = tr.x + tr.w/2; p2.y = tr.y + tr.h - 5*b;
		ps_move_to(gc, &p1);
		ps_line_to(gc, &p2);
	}
	ps_stroke(gc);
...
void ToolBar::OnCreate(uint32_t f, int x, int y, int w, int h)
{
	...
	m_backBtn.x = cur_loc; m_backBtn.y = 0; m_backBtn.w = box_width; 
	m_backBtn.h = box_height; cur_loc += (box_width+border);

	m_fordBtn.x = cur_loc; m_fordBtn.y = 0; m_fordBtn.w = box_width; 
	m_fordBtn.h = box_height; cur_loc += (box_width+border);

	m_zoomBtn.x = cur_loc; m_zoomBtn.y = 0; m_zoomBtn.w = box_width; 
	m_zoomBtn.h = box_height; cur_loc += (box_width+border);

	m_tabsBtn.x = cur_loc; m_tabsBtn.y = 0; m_tabsBtn.w = box_width;
   	m_tabsBtn.h = box_height; cur_loc += (box_width+border);

	m_configBtn.x = cur_loc; m_configBtn.y = 0; m_configBtn.w = box_width;
   	m_configBtn.h = box_height; cur_loc += (box_width+border);
