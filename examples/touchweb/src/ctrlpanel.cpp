...
void CtrlPanel::OnCreate(uint32_t f, int x, int y, int w, int h)
{
    ...
    m_hisBtn.x = cur_loc;
    m_hisBtn.y = titleHeight() + border;
    m_hisBtn.w = box_width;
    m_hisBtn.h = box_height;
    cur_loc += (box_width + border);

    m_favBtn.x = cur_loc;
    m_favBtn.y = titleHeight() + border;
    m_favBtn.w = box_width;
    m_favBtn.h = box_height;
    cur_loc += (box_width + border);

    m_fullBtn.x = cur_loc;
    m_fullBtn.y = titleHeight() + border;
    m_fullBtn.w = box_width;
    m_fullBtn.h = box_height;
    cur_loc += (box_width + border);

    m_newBtn.x = cur_loc;
    m_newBtn.y = titleHeight() + border;
    m_newBtn.w = box_width;
    m_newBtn.h = box_height;
    cur_loc += (box_width + border);

    cur_loc = DASH_TITLE_HEIGHT / 15;

    m_exitBtn.x = cur_loc;
    m_exitBtn.y = box_height + titleHeight() + border * 2;
    m_exitBtn.w = box_width;
    m_exitBtn.h = box_height;
    cur_loc += (box_width + border);

    m_confBtn.x = cur_loc;
    m_confBtn.y = box_height + titleHeight() + border * 2;
    m_confBtn.w = box_width;
    m_confBtn.h = box_height;
    cur_loc += (box_width + border);

    m_modeBtn.x = cur_loc;
    m_modeBtn.y = box_height + titleHeight() + border * 2;
    m_modeBtn.w = box_width;
    m_modeBtn.h = box_height;
    cur_loc += (box_width + border);

    m_searchBtn.x = cur_loc;
    m_searchBtn.y = box_height + titleHeight() + border * 2;
    m_searchBtn.w = box_width;
    m_searchBtn.h = box_height;
    cur_loc += (box_width + border);
