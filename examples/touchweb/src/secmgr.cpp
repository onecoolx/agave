/* secmgr.cpp - MaCross application
 *
...
void SecirtyData::clear_cookies(void)
{
	...
	Dialog::AlertBox(m_main, U("清除成功！"), U("信息"));
}
...
void SecirtyData::OnPaint(ps_context* gc, const Rect* r)
{
	...
	ps_wide_text_out_length(gc, border*8, DASH_TITLE_HEIGHT
						+cell_height/2-cell_height/4, (ps_uchar16*)U("清除Cookie"), 8);

	ps_wide_text_out_length(gc, border*8, DASH_TITLE_HEIGHT
						+cell_height+cell_height/2-cell_height/4, (ps_uchar16*)U("清除缓存"), 4);

	ps_wide_text_out_length(gc, border*8, DASH_TITLE_HEIGHT
						+cell_height*2+cell_height/2-cell_height/4, (ps_uchar16*)U("清除已保存密码"), 7);
