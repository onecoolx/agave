...
void HistoryView::OnPaint(ps_context* gc, const Rect* r)
{
	...
	if (!m_event.isEmpty()) {
		...
		if (tr.intersect(pr)) {
			...
			draw_button(gc, U("<<上一页"), brc, m_enableup, (m_pagebtn == 1) ? true : false);
		}
		...
		if (tr.intersect(pr)) {
			Rect brc = m_pagedown;
			brc.x -= scrollX(); brc.y -= scrollY();
			draw_button(gc, U("下一页>>"), brc, m_enabledown, (m_pagebtn == 2) ? true : false);
		}
...
void HistoryView::OnPaintContents(ps_context* gc, const Rect* r)
{
	...
	tr = m_pageup;
	if (tr.intersect(*r))
		draw_button(gc, U("<<上一页"), m_pageup, m_enableup, (m_pagebtn == 1) ? true : false);

	tr = m_pagedown;
	if (tr.intersect(*r))
		draw_button(gc, U("下一页>>"), m_pagedown, m_enabledown, (m_pagebtn == 2) ? true : false);
...
void HistoryManager::draw_button(ps_context* gc, const Rect& r, bool enable, bool highlight)
{
	...
	ps_wide_text_out_length(gc, r.x+r.w/2-b*12, r.y+r.h/2-b*6, (ps_uchar16*)U("清除"), 2);
