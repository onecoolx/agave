...
void FavoritesPanel::OnPaint(ps_context* gc, const Rect* d)
{
	...
	draw_button(gc, m_cancel, U("取消"), true, (m_btn == 1) ? true : false);
	draw_button(gc, m_commit, U("确定"), true, (m_btn == 2) ? true : false);
...
void FavoritesView::OnPaint(ps_context* gc, const Rect* r)
{
	...
	if (!m_event.isEmpty()) {
		...
		if (tr.intersect(pr)) {
			Rect brc = m_pageup;
			brc.x -= scrollX(); brc.y -= scrollY();
			draw_pagebtn(gc, U("<<上一页"), brc, m_enableup, (m_pagebtn == 1) ? true : false);
		}
		...
		if (tr.intersect(pr)) {
			Rect brc = m_pagedown;
			brc.x -= scrollX(); brc.y -= scrollY();
			draw_pagebtn(gc, U("下一页>>"), brc, m_enabledown, (m_pagebtn == 2) ? true : false);
		}
		ps_set_font(gc, of);
	}
}
...
void FavoritesView::OnPaintContents(ps_context* gc, const Rect* r)
{
	...
	tr = m_pageup;
	if (tr.intersect(*r))
		draw_pagebtn(gc, U("<<上一页"), m_pageup, m_enableup, (m_pagebtn == 1) ? true : false);

	tr = m_pagedown;
	if (tr.intersect(*r))
		draw_pagebtn(gc, U("下一页>>"), m_pagedown, m_enabledown, (m_pagebtn == 2) ? true : false);
	...
	if (e->type() == MouseEvent::MouseDown) {
		...
		if (m_drag) {
			...
			if (!m_event.isEmpty() && m_btn) {
				...
				if (Dialog::ConfirmBox(m_main, U("确定要删除吗?"), U("提示"))) {
...
void FavoritesManager::OnPaint(ps_context* gc, const Rect* r)
{
	...
	tr = m_listbtn;
	if (tr.intersect(*r)) {
		draw_button(gc, m_listbtn, U("查找"), true, (m_btn == 1) ? true : false);
	}
	...
	if (tr.intersect(*r)) {
		...
		draw_button(gc, m_addbtn, U("添加"), true, click);
	}
	ps_set_font(gc, of);
	ps_restore(gc);
}
